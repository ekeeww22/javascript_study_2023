...
for(let i=0; i<study.length; i++){console.log(study[i])}
// for ~in 문법 (DOM index number)
for(let i in study){console.log(study[i])}
// for ~of 문법 (DOM)
for(let i of study){console.log(i)}
console.log('---------------')
const boxA = document.querySelectorAll('.box a')
for(let i in boxA){console.log(i)} // DOM index + 기본속성 출력
for(let i of boxA){
